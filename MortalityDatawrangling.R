...
# 2. Database
####==========================================================================###
mortality_df <- read_excel("./data/MortalityRate.xlsx", sheet = "Sheet2") %>%
  clean_names() %>%
...
italy <- read.csv("./data/country/IT_data_mortality.csv", sep = ";")
germany <- read.csv("./data/country/DE_data_mortality.csv", sep = ";")
finland <- read.csv("./data/country/FI_data_mortality.csv", sep = ";")
hungary <- read.csv("./data/country/HU_data_mortality.csv", sep = ";")
romania <- read.csv("./data/country/RO_data_mortality.csv", sep = ";")
# Merge all country into one data set
country_info <- bind_rows(germany,italy,finland,hungary,romania) %>%
  rename("country" = "country_code",
         "zipcode" = "postal_code") %>%
  select(state, country, zipcode, mortality_rate)

#Merge country_info with mortality_df
...
df_merged %>%
  writexl::write_xlsx("./data/mortality_data_merged.xlsx")
